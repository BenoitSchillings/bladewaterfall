...
static int init_module(struct bladerf *dev, struct test_params *p, bladerf_module m)
{
    ...
    status = bladerf_set_bandwidth(dev,  m, 0.7 * p->samplerate, &bw_actual);
    ...
    bladerf_set_correction(dev, BLADERF_MODULE_RX, BLADERF_CORR_LMS_DCOFF_Q, 1*400); 
    return status;
}
...
void *rx_task(void *args)
{
	...
	pthread_mutex_unlock(&g_dev_lock);
	    ...
	    //fwrite (samples, 2, SIZE * 2, p_raw);
	    proc((short*)samples, SIZE * 2); 
            fix_fft(cvt, LSIZE);
            clean_fft(cvt);
